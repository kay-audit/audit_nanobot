    ...
    for col in df.columns:
        ...
        if any(kw in col_lower for kw in ("sum", "amt", "loss", "recovery", "потер", "ущерб", "возмещ", "сумм")) and not any(kw in col_lower for kw in ("id", "sid", "key", "код", "номер", "дата", "dt", "date", "type", "тип")):
            def _sum_numeric(series):
                from scripts.ior_hypothesis import _to_numeric_clean
                return _to_numeric_clean(series).sum()
            agg_dict[col] = _sum_numeric
        else:
            def _first_non_null(series):
                valid = series.dropna()
                valid = valid[valid.astype(str).str.strip() != ""]
                return valid.iloc[0] if not valid.empty else (series.iloc[0] if not series.empty else "")
