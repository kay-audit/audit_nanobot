"""
session_extract_manager.py — Кэш активной выгрузки сессии для appeals-analyzer.
...
# Хранилище сессионных датасетов в памяти: session_id -> { "df": pd.DataFrame, ... }
...
def set_session_extract(
    ...
    **kwargs
    ...
    """Сохраняет новую выгрузку для сессии, перезаписывая предыдущую (1 выгрузка на сессию)."""
    if not session_id:
    ...
    extract_data = {
        "df": df,
        "file_path": file_path,
        "skill_name": skill_name or "appeals-analyzer",
    ...
    logger.info(f"[appeals session_extract] Updated extract for session '{session_id}': {len(df) if df is not None else 0} rows.")
    return extract_data
...
def clear_session_extract(session_id: str) -> None:
    ...
    if session_id in _SESSION_EXTRACTS:
        ...
        logger.info(f"[appeals session_extract] Cleared extract for session '{session_id}'.")
